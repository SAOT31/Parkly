...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTab() {
        ...
        if(countEl) countEl.innerText = spots.length;
        if (!content) return;
        ...
        content.innerHTML = spots.map(s => `
            <article class="bg-card border border-border p-5 rounded-2xl flex gap-6 mb-4 animate-fade-in">
                <img src="${s.image}" class="w-32 h-24 rounded-xl object-cover bg-slate-800">
                <div class="flex-1">
                    <h4 class="font-bold text-white text-lg">${s.name}</h4>
                    <p class="text-xs text-slate-400 mb-2">${s.address}</p>
                    <div class="flex gap-2 mt-2 flex-wrap">
                        ${s.features ? s.features.map(f => `<span class="px-2 py-1 bg-slate-800 rounded text-[10px] text-slate-300">${f}</span>`).join('') : ''}
                    </div>
                </div>
                <div class="text-right">
                    <span class="block text-xl font-bold text-white">$ ${s.price}<span class="text-xs text-slate-500">/hr</span></span>
